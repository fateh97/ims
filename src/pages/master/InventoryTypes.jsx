...
export default function InventoryTypesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    const success = await addInventoryType(typeName);
    if (success) setTypeName("");
    setIsSaving(false);
  };
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="flex gap-4 mb-10">
          <input 
            ...
            placeholder="New Category Name (e.g. Bowling Ball)..."
          />
          <button 
            disabled={isSaving}
            className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-bold flex items-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 disabled:opacity-50"
          >
